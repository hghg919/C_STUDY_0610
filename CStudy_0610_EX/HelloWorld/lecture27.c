...
// 무엇을 만들지 정하기 어려운 분 : 별피하기, 콘솔 어드벤처 [구조체. 파일 저장 기능]
// 권장하는 부분 여러분이 만들고 싶은 프로젝트를 정하는 것 : 몬스터 데이터 프로그램, 도서 관리 프로그램, 테트리스, 공룡 달리기 게임
...
// void Swap : 인자로 넘겨주는 a와 b의 값만 변경해주고 종료된다.
...
// void* : 메모리 공간에 8바이트 확보햇는데, 어떤 형식인지 모른다. 쓰임이 있다.
// void 포인터를 사용하는 이유가 무엇일까? 
// IntSwap, DoubleSwap, FloatSwap 데이터 타입만 다른 버전의 함수들이 쓰이고 있다.
// 이 함수들을 타입을 지정해서 정해주는 함수로 만들면 하나로 표현할 수 있지 않을까?
...
// 열거형 ENUM 사용하는 이유
// 1. 코드의 가독성 증가 : 상수로 표현하는 변수의 이름이 정확히 어떤 내용있지 불완전합니다. enum 상수의 이름들을 정의하는 공간을 만드는 겁니다. TYPE -> 상수 3개 (Float, Int, Double) 
